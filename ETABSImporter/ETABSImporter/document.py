...
class document:
	...
	def plot(self, vertices:bool=True, frames:bool=True, areas:bool=True):
		...
		if frames:
			...
			fig.add_trace(go.Scatter3d(x=Ex, y=Ey, z=Ez, mode='lines', name='Frames'))
		...
		if areas:
			...
			for _,a in self.areas.items():
				t1 = [a.nodes[0], a.nodes[1], a.nodes[2]]
				t2 = [a.nodes[0], a.nodes[2], a.nodes[3]]
				for t in (t1, t2):
					i.append(t[0])
					j.append(t[1])
					k.append(t[2])
		...
		fig.add_trace(go.Mesh3d(
				x=[0,1,1,0], y=[0,0,1,1], z=[0,0,0,0], i=[0,0], j=[], k=k, 
				color = 'rgb(0,0,255)',
				name='Areas'))
