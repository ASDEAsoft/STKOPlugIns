...
if __name__ == "__main__":
    ...
    class TestWindow(QMainWindow):
        def __init__(self):
            super().__init__()
            self.setWindowTitle("Matplotlib Line Plot Widget Test")
            ...
            self.plot_widget = MplLinePlotContainer(
            ...
            # Add test button
            test_button = QPushButton("Add Test Data")
            test_button.clicked.connect(self.add_test_data)
            layout.addWidget(test_button)
            
            # Add clear button
            clear_button = QPushButton("Clear Plot")
            clear_button.clicked.connect(self.plot_widget.clear_plot)
            layout.addWidget(clear_button)
            
            self.line_count = 0
        
        def add_test_data(self):
            """Add some test data to the plot."""
            x = np.linspace(0, 10, 100)
            y = np.sin(x + self.line_count * 0.5)
            
            line_styles = ['-', '--', '-.', ':']
            ls = line_styles[self.line_count % len(line_styles)]

            self.plot_widget.add_line(x, y, ls)
            self.line_count += 1
